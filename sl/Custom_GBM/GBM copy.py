...
model = pickle.load(open('GBM.model', 'rb'))
...
def train_model(train_x, train_y, test_x, test_y):
    ...
    predtest = clf.predict(test_x)**2

    mae = mean_absolute_error(test_y**2, predtest)
    mse = mean_squared_error(test_y**2, predtest)
    rmse = np.sqrt(mean_squared_error(test_y**2, predtest))
