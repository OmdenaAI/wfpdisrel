...
def evaluate_input(filename, sid, windspeed):
    ## Evaluate the existance of the file
    if os.path.isfile(filename):
        try:
            data = pd.read_csv(filename)
            columns_list = data.columns.to_list()
            req_set = set(['SID', 'NAME', 'SEASON', 'LAT', 'LON', windspeed])
            ...
            if ((data['SID'] == sid).any()):
                ...
                raise Shape_Generator_Exception("Unable to find matching entries for {} sid".format(sid))
        except IOError as e:
...
class Shape_Generator:
    def __init__(self, file_path, sid, wind_speed, debug=False):
        """
        ...
        """
        evaluate_input(file_path, sid, wind_speed)
        self.dest_path = decide_target_loc(file_path)
        data_df = pd.read_csv(file_path)
        cyclones = data_df[['SID','NAME',
                                 'SEASON', 'LAT', 'LON', wind_speed]]
        # cyclones_filter = cyclones[(cyclones.year == cyclone_year) & (
        #                     cyclones.name == cyclone_name)]
        cyclones_filter = cyclones[cyclones.SID == sid]
        ...
        self.debug = debug
        self.extension = f"{self.sid}_{self.wind_speed}_"
        if self.debug:
    ...
    def generate_center_path(self, generateFile=False):
        ...
        """
        group_gdf = self.cyclone_gdf.groupby(['SID', 'SEASON'])[
...
class Population_Calculator(Shape_Generator):
    def __init__(self, file_path, sid, cyclone_year, wind_speed, debug=False):
        super().__init__(file_path, sid, wind_speed, debug=debug)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--sid", dest="sid", help="Ibtracs ID or SID", required=True)
    ...
    pop_object = Population_Calculator(args.filename, args.sid,args.year, args.windspeed, args.debug)
    output = ''
    if args.isbuffered:
